...
$confirmPassword = isset($_POST['confirmPassword']) ? $_POST['confirmPassword'] : ''; // Add this line
$secQuestion = isset($_POST['secQuestion']) ? $_POST['secQuestion'] : '';
$securityAns = isset($_POST['securityAns']) ? $_POST['securityAns'] : '';

if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["register"])) {
    // Validation checks
    $errors = [];

    if (empty($fullName) || empty($userName) || empty($email) || empty($password) || empty($secQuestion) || empty($securityAns)) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            // Check if confirmPassword is set before performing match check
            ...
            if (empty($errors)) {
                ...
                if ($stmt->execute()) {
                    ...
                    $mail->SMTPDebug = \PHPMailer\PHPMailer\SMTP::DEBUG_SERVER; // Set to 2 for detailed debug output
